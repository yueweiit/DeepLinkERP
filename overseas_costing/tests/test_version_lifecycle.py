...
class LifecycleFrappe:
    ...
    def get_all(self, doctype, *, filters, fields, **kwargs):
        ...
        return [{field: row.get(field) for field in fields} for row in self.records[doctype].values()
    ...
    def _doc(self, values):
        ...
        def insert(**kwargs):
            ...
            row = {key: value for key, value in vars(doc).items() if key not in {"doctype", "insert"}}
