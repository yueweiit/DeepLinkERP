...
def build_material_import_preview(existing: list, incoming: list, source: dict) -> dict:
    ...
    for index, source_row in enumerate(incoming or [], start=1):
        ...
        if len(candidates) == 1:
            target = candidates[0]
            status = "matched"
            target_key = target.get("stable_line_key") or ""
...
class FrappeMaterialImportRepository:
    ...
    def get_items(self, batch_name: str, version_name: str) -> list:
        fields = list(
            dict.fromkeys(
                ["name", "row_no", "stable_line_key", "source_doc_no"]
                + list(MATERIAL_IMPORT_FIELDS)
...
def apply_material_import(
    ...
    *,
    ...
    try:
        ...
        by_key = {str(item.get("stable_line_key") or ""): item for item in existing}
        pending_updates = []
