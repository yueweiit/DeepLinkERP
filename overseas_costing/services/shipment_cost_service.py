...
def shipment_value(row):
    valuation = object_json(row.get('extra_json')).get('shipment_valuation')
...
def project_summaries(rows):
    ...
    return [{k: (format(v, 'f') if k == 'gross_weight_kg' else format(v, '.2f')) if isinstance(v, Decimal) else v
