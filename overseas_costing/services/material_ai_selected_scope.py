...
def apply_scope(bundle, adoption, *, historical=False, store=None, ledger=None, batch_name=''):
    ...
    issues=[] if historical else dependency_issues(adoption,store=store,ledger=ledger,
        batch_name=batch_name or current.get('batch') or (bundle.get('batch') or {}).get('name',''), purpose='analysis')
    available=historical or bool(unchanged and not issues and now.get('available') and not now.get('invalid'))
    approved=bool(available and (original if historical else now).get('approved'))
    ...
    packing.update(selected_source=selected,available=available,approved=approved,completeness='unconfirmed',cost_version=current.get('cost_version'),dependency_issues=issues)
    ...
    source.update(goods=goods,goods_complete=False,selected_source=selected,scoped_text=dumps(goods),available=available,approved=approved,
