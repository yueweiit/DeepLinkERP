...
china_finance.bank_receipts.allocation_html = (suggestion, bank = {}) => {
	...
	const journal = lines.length ? `<p>所属时期：${e(suggestion.social_period)} · 计提及支付分录（CNY）</p><table class="table table-bordered receipt-journal"><thead><tr><th>摘要</th><th>科目</th><th>借方</th><th>贷方</th></tr></thead><tbody>${lines.map(r => `<tr><td>${e(r.summary)}</td><td>${e(r.account)}</td><td>${money(r.debit)}</td><td>${money(r.credit)}</td></tr>`).join("")}</tbody></table>` : "";
	return `<table class="table table-bordered receipt-allocation"><thead><tr><th>险种</th><th>总额（CNY）</th><th>公司比例</th><th>公司承担</th><th>个人承担</th></tr></thead><tbody>${suggestion.breakdown.map(r => `<tr><td>${e(r.item)}</td><td>${e(r.amount)}</td><td>${e(r.company_percent)}%</td><td>${e(r.company_amount)}</td><td>${e(r.personal_amount)}</td></tr>`).join("")}</tbody></table>${journal}`;
};
