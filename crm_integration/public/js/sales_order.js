...
frappe.ui.form.on("Sales Order", {
	...
	on_tab_change: function(frm) {
		if (!is_shipment_details_tab_active(frm) || frm._crm_shipment_loading) {
			return;
		}
		if (frm._crm_shipment_details) {
...
function render_shipment_details(frm) {
	...
	if (!frm || !frm.doc || frm.doc.__islocal || !frm.fields_dict.custom_shipment_details_html) {
		clear_shipment_details(frm);
		return;
	}

	const $wrapper = frm.fields_dict.custom_shipment_details_html.$wrapper;
	if (!$wrapper || !$wrapper.length) {
		return;
	}

	if (frm._crm_shipment_loading) {
		return;
	}
	frm._crm_shipment_loading = true;

	$wrapper.html(`<div class="text-muted small">${__("正在获取销售订单...")}</div>`);
	frappe.call({
		method: "crm_integration.crm_integration.sales_order_shipment.get_sales_order_shipment_details",
		args: {
			sales_order: frm.doc.name
		},
		callback: function(r) {
			frm._crm_shipment_loading = false;
			if (!r.message) {
				$wrapper.empty();
				return;
			}

			frm._crm_shipment_details = r.message;
			frm._crm_shipment_summary_page = frm._crm_shipment_summary_page || 1;
			frm._crm_shipment_detail_page = frm._crm_shipment_detail_page || 1;
...
function is_shipment_details_tab_active(frm) {
	...
	return active_tab && active_tab.df && active_tab.df.fieldname === "custom_shipment_details_tab";
}
...
function clear_shipment_details(frm) {
	...
	if (frm) {
		frm._crm_shipment_loading = false;
		frm._crm_shipment_details = null;
		frm._crm_shipment_summary_page = 1;
		frm._crm_shipment_detail_page = 1;
	}

	if (frm && frm.fields_dict && frm.fields_dict.custom_shipment_details_html) {
		frm.fields_dict.custom_shipment_details_html.$wrapper.empty();
	}
}
...
function render_shipment_details_html(frm) {
	...
	const $wrapper = frm.fields_dict.custom_shipment_details_html.$wrapper;
	$wrapper.html(build_shipment_details_html(frm, frm._crm_shipment_details || {}));
	$wrapper.find(".crm-shipment-download").on("click", function(e) {
		e.preventDefault();
		download_shipment_details(frm);
	});

	$wrapper.find(".crm-shipment-page-btn").on("click", function(e) {
		e.preventDefault();
		const target = $(this).data("target");
		const page = cint($(this).data("page"));
		if (target === "summary") {
			frm._crm_shipment_summary_page = page;
		} else if (target === "detail") {
			frm._crm_shipment_detail_page = page;
		}
		render_shipment_details_html(frm);
	});

	const data = frm._crm_shipment_details || {};
	const summary = data.summary || [];
	const delivery_notes = data.delivery_notes || [];
	const summary_page = get_shipment_page(summary, frm._crm_shipment_summary_page);
	const detail_page = get_shipment_page(delivery_notes, frm._crm_shipment_detail_page);

	const summary_container = $wrapper.find(".crm-shipment-summary-datatable").get(0);
	if (summary_container) {
		frm._crm_shipment_summary_dt = render_shipment_datatable(
			summary_container,
			get_shipment_summary_columns(),
			summary_page.rows.map((row) => [
				row.item_code,
				row.item_name,
				row.ordered_qty,
				row.delivered_qty,
	...
	const detail_container = $wrapper.find(".crm-shipment-detail-datatable").get(0);
	if (detail_container) {
		frm._crm_shipment_detail_dt = render_shipment_datatable(
			detail_container,
			get_shipment_detail_columns(),
			detail_page.rows.map((row) => [
				row.delivery_note,
				row.posting_date,
				row.item_code,
				row.item_name,
		...
		$wrapper.find(".crm-shipment-detail-datatable").on("click", ".crm-shipment-dn-link", function(e) {
			...
			if (dn) {
				frappe.set_route("Form", "Delivery Note", dn);
			}
		});
	}
}
...
function destroy_shipment_datatables(frm) {
	if (frm._crm_shipment_summary_dt && frm._crm_shipment_summary_dt.destroy) {
		frm._crm_shipment_summary_dt.destroy();
		frm._crm_shipment_summary_dt = null;
	}
	if (frm._crm_shipment_detail_dt && frm._crm_shipment_detail_dt.destroy) {
		frm._crm_shipment_detail_dt.destroy();
		frm._crm_shipment_detail_dt = null;
	}
}

function build_shipment_details_html(frm, data) {
	const summary = data.summary || [];
	const delivery_notes = data.delivery_notes || [];
	const summary_page = get_shipment_page(summary, frm._crm_shipment_summary_page);
	const detail_page = get_shipment_page(delivery_notes, frm._crm_shipment_detail_page);
	...
	return `
		<div class="crm-shipment-details-report">
			<div class="crm-shipment-details-header">
				<h5>${__("出货汇总")}</h5>
				<button type="button" class="btn btn-primary btn-sm crm-shipment-download">
					${__("下载 Excel")}
				</button>
			</div>
			<div class="crm-shipment-table-wrap">
				<div class="crm-shipment-summary-datatable"></div>
			</div>
			${build_shipment_pagination("summary", summary_page)}
			<h5 class="crm-shipment-detail-title">${__("销售出库明细")}</h5>
			<div class="crm-shipment-table-wrap">
				<div class="crm-shipment-detail-datatable"></div>
			</div>
			${build_shipment_pagination("detail", detail_page)}
		</div>
	`;
}
...
function get_shipment_summary_columns() {
	return [
		{
			id: "item_code",
			name: __("物料编码"),
		...
		},
		{
			...
			focusable: false,
			align: "right",
			format: (value) => format_shipment_qty(value),
		},
		{
			id: "delivered_qty",
			name: __("已出货数量"),
...
function get_shipment_detail_columns() {
	return [
		{
			id: "delivery_note",
			name: __("销售出库单"),
			...
			format: (value) => {
				...
				return `<a class="crm-shipment-dn-link" href="${route}" data-dn="${escape_html(value)}">${escape_html(value)}</a>`;
			},
		},
		{
			id: "posting_date",
...
function get_shipment_page(rows, current_page) {
	const page_size = 20;
	const total = rows.length;
	const total_pages = Math.max(Math.ceil(total / page_size), 1);
	const page = Math.min(Math.max(cint(current_page) || 1, 1), total_pages);
...
function build_shipment_pagination(target, page_info) {
	if (!page_info || page_info.total <= page_info.page_size) {
		return "";
	}

	const previous_page = Math.max(page_info.page - 1, 1);
	const next_page = Math.min(page_info.page + 1, page_info.total_pages);
	return `
		<div class="crm-shipment-pagination">
			<span class="text-muted small">
				${__("显示 {0}-{1} / 共 {2} 行", [page_info.start, page_info.end, page_info.total])}
			</span>
			<div class="btn-group btn-group-sm">
				<button type="button" class="btn btn-default crm-shipment-page-btn" data-target="${target}" data-page="${previous_page}" ${page_info.page <= 1 ? "disabled" : ""}>
					${__("上一页")}
				</button>
				<button type="button" class="btn btn-default crm-shipment-page-btn" data-target="${target}" data-page="${next_page}" ${page_info.page >= page_info.total_pages ? "disabled" : ""}>
					${__("下一页")}
				</button>
			</div>
		</div>
...
function download_shipment_details(frm) {
	if (!frm || !frm.doc || frm.doc.__islocal) {
		return;
	}

	open_url_post(
		"/api/method/crm_integration.crm_integration.sales_order_shipment.export_sales_order_shipment_details",
		{ sales_order: frm.doc.name }
	);
}
